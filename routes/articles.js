...
/* Get articles */
router.get('/articles', function getArticles (req, res) {
  var getArticlesQuery = "Select article_id,title,cover_img_path from article;";

  res.locals.connection.query(getArticlesQuery, function getArticlesQuery (error, results) {
    if (error != null) {
      console.log(error);
...
/* Get articles see */
router.get('/articles/see', function getArticlesSee (req, res) {
  if (req.urlParams.id) {
    var selectArticleQuery = 'SELECT * from article ' +
      "where article_id=" + req.urlParams.id + ";";

    res.locals.connection.query(selectArticleQuery, function (error, articleResults) {
      if (error != null) {
        console.log(error);
        res.render('articles', {'message': 'An error happened... Try again later!', 'is_admin': req.session.is_admin});
      }
      if (articleResults.length > 0) {
        ...
        res.locals.connection.query(selectCommentQuery, function (error, commentResults) {
          if (error != null) {
            console.log(error);
          }
          res.render('article', {
...
/* Get articles modify */
router.get('/articles/modify', function getArticlesModify (req, res) {
...
router.post('/articles/modify', function postArticlesModify (req, res) {
  if (req.session.is_admin) {
    if (req.urlParams.id) {
      ...
      res.locals.connection.query(updateArticleQuery, function (error, articleResults) {
        if (error != null) {
          console.log(error);
          res.render('articles', {
            'message': 'An error happened... Try again later!',
...
router.get('/articles/delete', function (req, res) {
  if (req.session.is_admin) {
    if (req.urlParams.id) {
      ...
      res.locals.connection.query(deleteArticleQuery, function (error, articleResults) {
        if (error != null) {
          console.log(error);
          res.render('articles', {
            'message': 'An error happened... Try again later!',
